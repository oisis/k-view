...
- **Authentication**: Implements a secure OAuth2/OIDC flow integrated with Google SSO, alongside a **Local Authentication** provider for static users (bcrypt-hashed passwords).
- **RBAC**: A declarative role-mapping system managed via `assignments.yaml` (injected via ConfigMap) or environment variables. It maps emails/groups/usernames to internal roles (`kview-cluster-admin`, `viewer`), which translate to Kubernetes impersonation.
...
- **User Interface**: Features a modern **Glassmorphism** aesthetic with a unified CSS variable system for themes, powered by **Tailwind CSS** and **Shadcn/ui**.
- **Visualization**: 
  - **React Flow**: Dynamically generates interactive, draggable network flow diagrams (Ingress -> Service -> Pod).
...
1. **Frozen Views**: A dedicated automated testing framework using **Playwright** ensures that resource lists and detailed views remain consistent across changes.
2. **Unified Test Suite**: A complete set of 27 complex manifests in `/examples/test-suite` provides a predictable environment for UI verification.
...
## Security Model
1. **Stateless**: No database is used. All state is derived from Kubernetes or OIDC/Local Auth.
