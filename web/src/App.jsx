...
function Section({ label, children, defaultOpen = true, storageKey }) {
    const key = storageKey || `sidebar-section-${label}`;
    ...
    return (
        <div>
            ...
            >
                <span className="text-[11px] font-semibold tracking-widest uppercase text-gray-500 group-hover:text-gray-400 transition-colors">
                    {label}
                </span>
                {open
...
function NavItem({ href, icon: Icon, label, active }) {
    return (
        ...
        >
            ...
            {active && <ChevronRight size={11} className="text-blue-400" />}
        </a>
    );
}
        ...
        <aside className="w-56 bg-gray-800 border-r border-gray-700 flex-col hidden md:flex h-full shrink-0">
            ...
            <div className="px-3 py-3 border-t border-gray-700 space-y-1.5">
                ...
                {user.role === 'admin' && (
                    <a
                        href="/admin"
                        className={`flex items-center gap-2.5 px-2 py-1.5 rounded text-sm transition-colors w-full
              ${p === '/admin'
                ...
                <button
                    onClick={onLogout}
                    className="w-full flex items-center gap-2 justify-center py-1.5 rounded text-sm bg-gray-700/40 text-gray-400 hover:bg-gray-700 hover:text-white transition-colors"
                >
